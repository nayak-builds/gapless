...
| POST | `/resume/parse` | MVP | Multipart `file` (PDF / Markdown / txt, max 5 MB). Auth + LLM rate limit. Extracts text (empty/scanned PDF → 422). Groq JSON `{ "skills": [{ "name" }] }`. Inserts `resumes.raw_text` for the caller (new row each upload). Returns `{ "skills": [{ "name" }] }` |
| POST | `/match-score` | explicit | Body `{ "jd_id": "<uuid>" }`. Auth. 404/403 on JD. 422 if no `resumes` row (`Upload a resume first to get a match score`) or no `gaps` yet. Score is **Python-only** from `gaps` + `skills_required` importance (required = 1.0, nice-to-have = 0.5). Groq rewrite suggestions for **missing** skills only, rate-limited when Groq runs. Server drops any quote that is not a normalized substring of stored `raw_text`. Returns `{ "score", "matched_count", "total_count", "suggestions": [{ "skill", "original_quote", "suggested_rewrite" }] }` |
| POST | `/gaps/compute` | MVP | Body `{ "jd_id": "<uuid>" }`. 404 if missing, 403 if another user's JD. Name match vs owned skills, then optional resume evidence (substring + cached Groq quotes; Groq only on cache miss). Replaces `gaps` for that user+JD. Returns `{ "matched", "missing", "score", "matched_count", "total_count" }` (`gap_level` is `none` or the required skill's importance; `match_source` is `owned` or `resume` on matched rows). `score` is the same Python weighted formula as `/match-score` (no extra Groq) |
...
| POST | `/quiz/generate` | V2 | Body `{ "gap_id": "<uuid>" }`. Auth required. Looks up the caller's gap, retrieves top 3 pgvector chunks for `skill_name` (skill must match note title/chunk, or cosine distance ≤ 0.45). 422 `{ "error": "No notes found for this skill yet — add some notes first" }` if none. Otherwise Groq 3–5 MCQs grounded in those chunks. Rate-limited. Does **not** persist. Returns `{ "gap_id", "skill_name", "questions": [{ "prompt", "options", "correct_index", "explanation" }] }` |
